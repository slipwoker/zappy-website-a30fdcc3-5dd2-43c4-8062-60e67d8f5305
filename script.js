document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    contactForm.addEventListener('submit', function (e) {
      let isValid = true;
      const submitableFields = contactForm.querySelectorAll('input:not([type="submit"]):not([type="reset"]):not([type="button"]):not([type="hidden"]), textarea, select');

      submitableFields.forEach(function (field) {
  ...
  const ANIMATION_SELECTORS = '.animate-on-scroll, .feature-card, .pricing-card, .testimonial-card, .team-card, .service-card, .portfolio-item, .blog-card, .stat-item, .step-item';
        ...
        entries.forEach(function (entry) {
          if (entry.isIntersecting) {
            ...
            const siblings = Array.from(el.parentNode.children).filter(function (child) {
              return child.classList.contains(ANIMATION_CLASS) && !child.classList.contains(VISIBLE_CLASS);
            });
  ...
  style.textContent = [
    '.' + ANIMATION_CLASS + '{opacity:0;transform:translateY(28px);transition:opacity 0.6s ease,transform 0.6s ease;}',
  ...
  const mutationObserver = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
      mutation.addedNodes.forEach(function (node) {
        if (node.nodeType === 1) {
          if (node.matches && node.matches(ANIMATION_SELECTORS)) {
            if (!node.classList.contains(ANIMATION_CLASS)) {
              node.classList.add(ANIMATION_CLASS);
            }
          }
          const children = node.querySelectorAll ? node.querySelectorAll(ANIMATION_SELECTORS) : [];
    ...
    "translations": {
      "en": {
        "consentModal": {
          ...
          "description": "השבחת נכסים שיפוץ ועיצובים פנים uses cookies to enhance your experience, analyze site usage, and assist in our marketing efforts. You can manage your preferences anytime.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'a30fdcc3-5dd2-43c4-8062-60e67d8f5305',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'a30fdcc3-5dd2-43c4-8062-60e67d8f5305';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'a30fdcc3-5dd2-43c4-8062-60e67d8f5305';
